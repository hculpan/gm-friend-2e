<template>
  <section id="npcs" class="bg-dark text-light">
    <form @submit.prevent="formSubmitted" class="justify-content-center">
      <div class="h3">Edit NPC</div>
      <div class="grid-container mt-3">
        <label>Name</label>
        <input type="text" class="rounded border-0" v-model="npc.name" />
        <label>Location</label>
        <div class="d-flex justify-content-between">
          <select name="location" id="location" class="rounded border-0 flex-grow-1" v-model="npc.location">
            <option value=""></option>
        ...
        <label>Notes</label>
        <textarea name="notes" id="notes" cols="30" rows="10" class="rounded border-0" v-model="npc.notes"></textarea>
      </div>

      <div class="mt-3">
        <button class="btn btn-primary me-2" :disabled="!npc.name || !npc.location">Save</button>
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      if (error) {
        ...
        var { result, error } = await getNpc(route.params.npcId);
